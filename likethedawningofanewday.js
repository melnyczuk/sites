...
//  AT RESIZE
$(window).resize(resizeWindow = function resizeWindow() {
    w = $(window).width();
    h = $(window).height();
...
$(document).on('click', function(event) {
    ...
    imgSwitch();
    //textRandom();
    $('body').css('background-color', 'rgb(' + count + ',' + count + ',' + count + ')');
});

function textRandom() {
    for (var i = 0; i < $text.length; i++) {
        var ls = Math.floor(Math.random() * (w * 0.1)) + $text[i].css('letter-spacing').length;
        var lp = Math.floor(Math.random() * (w * 0.55)) + $text[i].css('left').length;
        console.log(lp);
        $text[i].css({
            letterSpacing: ls,
            left: lp
